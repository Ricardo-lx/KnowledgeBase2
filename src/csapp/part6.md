...
arrangements,	the	general	principles	are	the	same.	For	our	discussion	in
...
of	the	memory	blocks	that	map	to	that	set.	So	the	cache	must	search
...
only	when	it	is	evicted	from	the	cache	by	the	replacement	algorithm.
...
strides—yet	another	reason	to	favor	sequential	stride-1	accesses	in	your
code.
...
the	L1	cache,	and	to	exploit	spatial	locality	so	that	as	many	words	as
...
programs	by	writing	programs	with	good	spatial	and	temporal	locality.
...
blocks.	You	are	given	the	following	definitions:
...
where	the	arguments	to	the	procedure	are	pointers	to	the	destination
(
)	and	source	(
...
The	point	of	this	little	drill	is	to	make	sure	you	understand	the	relationship
...
This	is	a	simple	problem	that	will	give	you	some	interesting	insights	into
...
Each	16-byte	cache	line	holds	two	contiguous	
	structures.
...
MB/s,	the	clock	frequency	is	2,100	MHz,	and	the	individual	read

accesses	are	in	units	of	8-byte	longs.	Thus,	from	this	graph	we	can
...
Linking	is	usually	handled	quietly	by	the	linker	and	is
...
What	does	it	really	mean	when	you	define	a
...
The	driver	goes	through	the	same	process	to	generate	
	Finally,	it
...
with	the	necessary	system	object	files,	to	create	the	binary	
executable
object	file
...
which	copies	the	code	and	data	in	the	executable	file	
	into	memory,
and	then	transfers	control	to	the	beginning	of	the	program.
...
program	take	as	input	a	collection	of
...
keep	in	mind	some	basic	facts	about	linkers:	Object	files	are	merely

collections	of	blocks	of	bytes.	Some	of	these	blocks	contain	program
...
generate	the	object	files	have	already	done	most	of	the	work.
...
).	Although	our	discussion	will

focus	on	ELF,	the	basic	concepts	are	similar,	regardless	of	the	particular
format.
Figure	
7.3	
...
statements,	and	jump	tables	for	switch	statements.
...
or	static	variables	that	are	initialized	to	zero.	This	section	occupies	no

actual	space	in	the	object	file;	it	is	merely	a	placeholder.	Object	file
...
global	variable	will	need	to	be	modified.	On	the	other	hand,

instructions	that	call	local	functions	do	not	need	to	be	modified.	Note
...
table	is	a	sequence	of	null-terminated	character	strings.
...
space	in	
	or	
	for	each	definition	and	creates	a	local	linker

symbol	in	the	symbol	table	with	a	unique	name.	For	example,	suppose	a
...
assign	symbols	in	relocatable	object	files	to	COMMON	and	
...
the	following	source	file	on	a	Linux	machine,

then	the	compiler	runs	without	a	hitch,	but	the	linker	terminates	when	it
...
Happily,	C++	and	Java	use	compatible	mangling	schemes.	A
...
following	rules	for	dealing	with	duplicate	symbol	names:
...
symbol	main	is	defined	multiple	times	(rule	1):
...
choose	the	strong	symbol	defined	in	the	other	(rule	2):
...
The	same	thing	can	happen	if	there	are	two	weak	definitions	of	
	(rule
3):
″
″

The	application	of	rules	2	and	3	can	introduce	some	insidious	run-time
...
GCC
	...
	flag,	which	triggers	an	error	if	it	encounters	multiply-defined

global	symbols.	Or	use	the	
...
So	far,	we	have	assumed	that	the	linker	reads	a	collection	of	relocatable
...
programmers	could	link	into	their	executables:
...
executables,	a	process	that	would	be	error	prone	and	time	consuming:

The	notion	of	a	static	library	was	developed	to	resolve	the	disadvantages
...
needs	to	include	the	names	of	a	few	library	files.	(In	fact,	C	compiler
...
To	build	the	executable,	we	would	compile	and	link	the	input	files	
and	
:
-------------------------------------------
code/link/main2.c
...
linker	also	copies	the	
	module	from	
,	along	with	a	number
of	other	modules	from	the	C	run-time	system.
...
U
	and	
D
	no	longer	change.	At	this	point,	any	member	object

files	not	contained	in	
E
...
the	other	hand,	the	libraries	are	not	independent,	then	they	must	be

ordered	so	that	for	each	symbol	
s
...
the	static	linker	to	resolve	all	symbol	references.
...
Programs	larger	than	2	GB	can	be	compiled	using	the	
(
medium	code	model
)	and	
	(
...
absolute	addressing,	then	it	is	relocated	by	lines	11−13.
...
,
which	is	the	address	of	the	instruction	immediately	following	the	

instruction.	To	execute	the	call	instruction,	the	CPU	performs	the
following	steps:
1
.	
Push	PC	onto	stack
...
the	instructions	without	any	further	modifications.
...
Gaps	due	to	segment	alignment	requirements	and	address-space	layout
...
initializes	the	execution	environment,	calls	the	user-level	
	function,
handles	its	return	value,	and	if	necessary	returns	control	to	the	kernel.
...
,	and
memory	mapping
,	which	we	haven't	discussed	yet.	As	we

encounter	these	concepts	later	in	
Chapters	
8
	and	
9
...
more	efficient	and	sophisticated	approach	based	on	dynamic	linking.

The	idea	is	to	package	each	function	that	generates	dynamic	content
...
immediately,	or	the	
	flag,	which	instructs	the	linker	to	defer

symbol	resolution	until	code	from	the	library	is	executed.	Either	of	these
values	can	be	
OR
ed	with	the	
	flag.
...
shared	module's	code	segment	can	be	shared	by	an	unlimited	number	of

processes.	(Of	course,	each	process	will	still	get	its	own	copy	of	the
...
module.	However,	if	
	were	defined	by	another	shared	module,	then

the	indirect	access	through	the	GOT	would	be	necessary.	In	this	case,
...
procedure
linkage	table	(PLT)
.	If	an	object	module	calls	any	functions	that	are

defined	in	shared	libraries,	then	it	has	its	own	GOT	and	PLT.	The	GOT	is
...
these	entries	is	responsible	for	invoking	a	specific	function.	
...
program	for	manipulating	shared
libraries:
...
references	is	another	source	of	confusing	link-time	errors.

Loaders	map	the	contents	of	executable	files	into	memory	and	run	the
...
link,	and	access	the	functions	and	data	in	shared	libraries.
...
For	each	symbol	that	is	defined	and	referenced	in	
,	indicate	if	it	will
...
When	this	program	is	compiled	and	executed	on	an	x86-64	Linux	system,
...
Consider	the	call	to	function	swap	in	object	file	
	(
Problem	
7.6
).
...
the	various	tools	for	manipulating	object	files.
...
This	is	a	simple	drill	that	checks	your	understanding	of	the	rules	that	a
...
relative	references.	You	were	given	that
...
processes	that	create	child	processes	must	be
...
important	ideas,	you	need	to	understand	ECF.
...
execution	of	an	application	program;	processes

and	threads	whose	execution	overlap	in	time;
...
operating	system.	We	also	discuss	system	calls,

which	are	exceptions	that	provide	applications	with
...
but	this	is	usually	unnecessary	because	the	meaning	is	clear	from
the	context.
Figure	
8.1	
...
kind	of	event.	When	the	exception	handler	finishes	processing,	one	of

three	things	happens,	depending	on	the	type	of	event	that	caused	the
exception:
1
.	
...
The	exception	table	is	a	jump	table	where	entry	
k
...
detects	that	an	event	has	occurred	and	determines	the	corresponding
...
software	by	the	exception	handler.	After	the	handler	has	processed	the

event,	it	optionally	returns	to	the	interrupted	program	by	executing	a
...
application	program's	control	flow.
...
controlled	access	to	such	kernel	services,	processors	provide	a	special
...
n
.	Executing	the	
	instruction	causes	a	trap	to	an

exception	handler	that	decodes	the	argument	and	calls	the	appropriate
kernel	routine.	
Figure	
8.6
...
system.	Numbers	in	the	range	from	32	to	255	correspond	to	interrupts
...
Linux	provides	hundreds	of	system	calls	that	application	programs	use
...
calls.	Each	system	call	has	a	unique	integer	number	that	corresponds	to

an	offset	in	a	jump	table	in	the	kernel.	(Notice	that	this	jump	table	is	not
...
or	other	applications	in	the	context	of	the	new	process.

A	detailed	discussion	of	how	operating	systems	implement	processes	is
...
represents	a	portion	of	the	logical	flow	for
...
parallel	flows
.	If	two	flows
are	running	concurrently	on	different	processor	cores	or	computers,	then
...
When	the	mode	bit	is	not	set,	the	process	is	running	in	
user	mode
.	A
...
switch	to	another	process.	For	example,	if	a	
	system	call	requires	a

disk	access,	the	kernel	can	opt	to	perform	a	context	switch	and	run
...
processor	after	the	disk	controller	has	finished	transferring	the	data	from
disk	to	memory.
...
continues	to	run	until	the	next	exception	occurs,	and	so	on.
...
Given	this	function,	our	call	to	
...
Given	this	wrapper,	our	call	to	
...
PID	of	the	child.	In	the	child,	
	returns	a	value	of	0.	Since	the	PID	of

the	child	is	always	nonzero,	the	return	value	provides	an	unambiguous
...
processes	that	run	concurrently.	The	instructions	in	their	logical

control	flows	can	be	interleaved	by	the	kernel	in	an	arbitrary	way.
...
of	the	instructions	in	different	processes.
---------------------------------------------------------------------------------------------
---------
code/ecf/fork.c
...
be	labeled	with	information	such	as	the	current	value	of	a	variable.
...
and	half	dead.	A	zombie	process	is	similar	in	the	sense	that

although	it	has	already	terminated,	the	kernel	maintains	some	of
...
If	the	
	...
	encodes	status
information	about	the	child	that	caused	the	return	in	status,	which	is	the
...
Constants	such	as	WNOHANG	and	WUNTRACED	are	defined	by
...
for	all	of	the	functions	used	in	the	book.	The	
	header	file	is
available	online	from	the	CS:APP	Web	site.
-------------------------------------------------------------------------------------------------
-----
code/ecf/waitpid1.c
...
function	terminated	normally,	and	prints	an	error	message	otherwise.

When	we	run	the	program	on	our	Linux	system,	it	produces	the	following
output:
...
The	sleep	function	suspends	a	process	for	a	specified	period	of	time.
	...
	returns	zero	if	the	requested	amount	of	time	has	elapsed,	and	the
number	of	seconds	still	left	to	sleep	otherwise.	The	latter	case	is	possible
if	the	
	function	
...
The	argument	list	is	represented	by	the	data	structure	shown	in	
Figure
8.20
.	The	
...
the	main	routine	of	the	new	program,	which	has	a	prototype	of	the	form
...
If	the	environment	array	contains	a	string	of	the	form	
,	then
	deletes	it	and	
	replaces	
oldvalue
