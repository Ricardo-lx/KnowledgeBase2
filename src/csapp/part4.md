...
different	versions.	Each	of	these	extensions	manages
...
allows	data	to	be	stored	in	16	YMM	registers,	named	
.	Each
...
generates	similar	code	for	converting	from	double	precision	to	single
precision:
...
helpful	to	examine	the	assembly	code	before,	during,	and	after	the
...
signedness	and	size,	as	well	as	integral	promotion,	add	challenge	to
this	problem.
Instruction
Comments
Read	8	bytes
...
This	exercise	is	intended	to	reinforce	your	understanding	of	how	loops
are	implemented.
Solution	to	Problem	
3.28	
(page
...
required	8,192	chips,	whereas	the
...
builds	on	our	discussion	of	Boolean	algebra	and	bit-
...
program	registers:
	...
	15	to	simplify	the
instruction	encoding.)	Each	of	these	stores	a	64-bit	word.	Register	
is	used	as	a	stack	pointer	by	the	push,	pop,	call,	and	return	instructions.

Otherwise,	the	registers	have	no	fixed	meanings	or	values.	There	are
three	single-bit	
condition	codes
,	
,	and	
...
Explicitly	identifying	the	four	types	of	data	transfer	will	prove	helpful
...
register	operands,	such	as	branches	and	call,	do	not	have	a	register

specifier	byte.	Those	that	require	just	one	register	operand	(
,	and	
)	have
Figure	
4.3	
...
.	We	write	this	in	byte-reversed	order	as	

.	Combining	these,	we	get	an	instruction	encoding	of
...
into	individual	instructions.	This	causes	problems	for	disassemblers	and

other	tools	that	attempt	to	extract	machine-level	programs	directly	from
...
a	much	simpler	form	of	instruction	set.	In	fact,	many	of	the	high-

level	instructions	that	were	being	added	to	instruction	sets	were
...
Variable-size	encodings.	x86-64
...
used	in	embedded	applications,	such	as	cell	phones.

In	the	early	1990s,	the	debate	diminished	as	it	became	clear	that
...
Intel	and	other	x86	processor	manufacturers	could	overcome	the

inefficiencies	created	by	the	original	8086	instruction	set	design,
...
The	x86-64	code	was	generated	by	the	
GCC
	
compiler.	The	Y86-64	code	is
...
from	memory	and	add	it	to	a	register,	whereas	the	x86-64	code	can

do	this	with	a	single	
	instruction	(line	5).
...
The	label	array	denotes	the	start	of	this	array,	and	is	aligned	on	an	8-byte
...
of	either	simulating	the	hardware	or	running
...
memory	values,	it	only	prints	out	words	that	change	during	simulation,

either	in	registers	or	in	memory.	The	original	values	(here	they	are	all
...
simpler	approach	is	to	conduct	an

experiment	on	an	actual	machine.	The	C	compiler	would	not
...
on	page	178	describes	how	to	write
...
question	to	ask	is,	"Why	worry	about	such	picky	details?"
...
We	conclude,	therefore,	that	working	out	details	in	advance	and
...
(HDL),	a	textual	notation	that	looks	similar	to	a	programming

language	but	that	is	used	to	describe	hardware	structures	rather
...
gate	with	inputs	a,	b,	and	c	is	described
...
gate)	or	are	both	0	(detected	by	the	lower	
AND
	
gate).	We
write	the	function	of	this	network	in	HCL	as

This	code	simply	defines	the	bit-level	(denoted	by	data	type	
)	signal
...
the	value	of	a	control	input	signal.	In	this	single-bit	multiplexor,	the	two

data	signals	are	the	input	bits	a	and	b,	while	the	control	signal	is	the	input
...
gate	passes	signal	a	when	s	is	1.	Again,	we	can	write	an
...
from	word	B.	Word-level	equality	is	one	of	the	operations	in	HCL.
...
and	so	the	functionality	of	the	circuit	shown	in	
Figure	
4.12
	can	be
...
generating	!s	once	and	reusing	it	at	each	bit	position.
...
expression	into	hardware,	a	logic	synthesis	program	would	need	to
...
data	input	is	transmitted	to	the	output.
...
Combinational	circuits,	by	their	very	nature,	do	not	store	any	information.
...
pass	a	word	directly	from	one	instruction	to	another	to
...
a	manner	similar	to	the	loading	of	values	into	a	clocked	register.	Every

time	the	clock	rises,	the	value	on	input	valW	is	written	to	the	program
...
address	to	the	desired	address,	data	in	to	the	desired	value,	and	write	to
1.	When	we	then	operate	the	clock,	the	specified	location	in	the	memory
will	be	updated,	as	long	as	the	address	is	valid.	As	with	the	read

operation,	the	error	signal	will	be	set	to	1	if	the	address	is	invalid.	This
...
Figure	
4.17
	to	illustrate	the	processing	of	different	Y86-
...
tables	describes	an	assignment	to	some	signal	or	stored	state
...
x86-64).	For	example,	the	instruction	
	is	supposed	to
compute	the	value	
.	Nothing	happens	in	the	memory

stage	for	these	instructions,	but	valE	is	written	to	register	rB	in	the	write-
...
but	we	will	see	that	having	the	stack	pointer	as	both	valA	and	valB	makes

the	subsequent	flow	more	similar	to	that	of	other	instructions,	enhancing
...
What	effect	does	this	instruction	execution	have	on	the	registers	and	the
PC?
Practice	Problem	
4.15	
(solution	page	
...
address	of	the	following	instruction)	if	the	flag	is	0.	Our	notation	
x
	?	
a
	:	
...
moves.	Show	how	you	would	modify	the	steps	for	the	
...
consists	of	9	bytes.	The	first	has	value	
...
counter	(PC),	shown	in	the	lower	left-hand	corner	of	the	figure.
...
of	the	control	logic	blocks,	shown	as	gray	rounded	boxes
...
the	data	memory,	and	the	register	file.	These	are	controlled	via	a	single

clock	signal	that	triggers	the	loading	of	new	values	into	the	registers	and
...
though	the	condition	codes	are	not	set	until	the	clock	rises	to	begin	the

next	clock	cycle,	they	will	be	updated	before	any	instruction	attempts	to
read	them.
Figure	
4.25
...
to	the	different	instructions	being	executed.	We	assume	the	processing

starts	with	the	condition	codes,	listed	in	the	order	
,	and	
,	set	to
...
the	state	elements	to	start	the	next	cycle.
...
on	page	472.	We	show	some	example	blocks
...
register	IDs,	ALU	operations,	and	status	codes.
...
decode	and	write-back	stages	in	SEQ.	These	two	stages	are	combined

because	they	both	access	the	register	file.
...
addresses	(two	read	and	two	write)	used	by	the	register	file.	The	values

read	from	the	register	file	become	the	signals	valA	and	valB.	The	two
...
moment	the	conditional	move	instructions,	then	we	can	combine	the

destination	registers	for	all	of	the	different	instructions	to	give	the
...
priority
	among	the	two	write	ports	so

that	when	both	attempt	to	write	the	same	register	on	the	same	cycle,	only
...
express	the	behavior	of	the	control	block	that	generates	aluA	as	follows:
...
however,	we	want	it	to	use	the	operation	encoded	in	the	ifun	field	of	the
...
control	logic.	We	omit	the	detailed	design	of	this	unit.
...
blocks	generate	the	control	signals	indicating	whether	to	perform	a	read

or	a	write	operation.	When	a	read	operation	is	performed,	the	data
...
4.18
	to	
4.21
show,	the	new	PC	will	be	valC,	valM,	or	valP,	depending	on	the

instruction	type	and	whether	or	not	a	branch	should	be	taken.	This
...
within	a	single	cycle.	As	an	example,	consider	the	processing	of	a	
...
block	in	the	figure	is	implemented	as	combinational	logic,	meaning	that

the	signals	will	pass	through	a	series	of	logic	gates,	with	the	outputs
...
and	B,	while	instruction	
	(shown	in	light	gray)	begins	propagating

through	stage	A	(points	2	and	3).	Just	before	the	clock	rises	again,	the
...
register	inputs	would	not	yet	be	valid	when	the	clock	rises.

As	with	our	discussion	of	the	timing	for	the	SEQ	processor	(
Section
4.3.3
...
would	increase	to	510	ps	due	to	the	slower	clock	rate.
...
4.32
	and
determine	that	it	can	be	separated	into	a	sequence	of	six	blocks,

named	A	to	F,	having	delays	of	80,	30,	60,	50,	70,	and	10	ps,
...
we	have	divided	the	computation	into	six	stages,	each	requiring	50	ps.

Inserting	a	pipeline	register	between	each	pair	of	stages	yields	a	six-
...
of	300/
k
,	and	with	each	pipeline	register	having	a	delay	of	20	ps.
...
(line	2);	and	this	instruction	stores	its	result	in	
,	which	must	then	be
read	by	the	
	instruction	(line	3).
...
the	pipeline	stages.	We	refer	to	this	modified	design	as	SEQ+.
...
can	implement	a	processor	in	a	way	that	differs	from	the

conceptual	model	implied	by	the	ISA,	as	long	as	the	processor
...
that	we	will	deal	with	shortly.
...
expanded	view	for	cycle	5	shows	the	pipeline	stages	with	the	fetch	stage
...
D_stat,	E_stat,	M_stat,	and	W_stat.	We	also	need	to	refer	to	some

signals	that	have	just	been	computed	within	a	stage.	These	are	labeled
...
With	our	naming	system,	the	uppercase	prefixes	`D',	`E',	`M',	and
`W	refer	to	pipeline	
registers
,	and	so	M_stat	refers	to	the	status
code	field	of	pipeline	register	M.	The	lowercase	prefixes	`f',	`d',	`e',
`m',	and	`w'	refer	to	the	pipeline	
stages
...
the	operation	of	our	pipelined	processors.
...
covered	the	implementation	of	normal	instruction	execution.	Suffice	it	to
...
branches	are	used	for	conditional	operations,	and	these	are	less

likely	to	be	taken.	In	Problems	4.55	and	4.56,	you	can	modify	the
...
prediction	was	incorrect	and	therefore	we	have	fetched	and	partially

executed	the	wrong	instructions.	We	will	return	to	this	matter	in	
Section
4.5.8
.
...
return	instruction	is	fetched,	the	top	value	is	popped	from	this

stack	and	used	as	the	predicted	return	address.	Like	branch
...
introducing	pipelining	into	a	system	with	feedback	can	lead	to	problems
...
activities	during	this	cycle	is	shown	at	the	bottom	of	the	figure.	The	first
	instruction	has	passed	through	the	write-back	stage,	and	so
...
stage.	Unfortunately,	the	pending
...
memory	stage.	By	the	time	a	conditional	move	or	jump

reaches	the	execute	stage,	any	preceding	integer	operation
...
has	the	same	effect	as	having	three	
	instructions	between	the	second

	instruction	and	the	
	instruction.	This	mechanism	can	be
implemented	fairly	easily	(see	
Problem	
4.53
),	but	the	resulting
...
pending	write	to	
	on	write	port	E.	It	can	therefore	avoid	stalling	by
...
only	needs	to	generate	signals	valA	and	valB	by	the	end	of	the	clock

cycle	so	that	pipeline	register	E	can	be	loaded	with	the	results	from	the
...
and	"Fwd	B"	in	the	decode	stage.	The	block	labeled	"Sel+Fwd	A"

combines	the	role	of	the	block	labeled	"Select	A"	in	PIPE—	with	the
...
instruction	passes	through	the	execute	stage,	the	pipeline	control	logic

detects	that	the	instruction	in	the	decode	stage	(the	
)	requires	the
...
normally	continue	flowing	through	the	pipeline.
...
hazards	without	stalling	the	pipeline.
...
Control	hazards	arise	when	the	processor	cannot	reliably	determine	the
...
in	cycle	3,	and	the	instruction	following	this	one	will	be	fetched	in	cycle	4.

By	the	time	the	branch	logic	detects	that	the	jump	should	not	be	taken
...
therefore	not	have	any	effect	on	the	programmer-visible	state.	The	only

drawback	is	that	two	clock	cycles'	worth	of	instruction	processing
...
processor	pipeline.	Our	handling	of	the	three	internal	exceptions	gives

just	a	glimpse	of	the	true	complexity	of	correctly	detecting	and	handling
exceptions.
...
exception	should	be	reported	to	the	operating	system.
...
state	before	the	excepting	instruction	completes.	For	example,	consider

the	following	code	sequence,	in	which	we	assume	that	user	programs	are
...
that	instruction,	until	it	reaches	the	write-back	stage.	At	this	point,	the

pipeline	control	logic	detects	the	occurrence	of	the	exception	and	stops
execution.
...
together	with	all	other	information	about	an	instruction	through	the

pipeline	provides	a	simple	and	reliable	mechanism	for	handling
exceptions.
4.5.7	
...
srcA	signal	in	SEQ	to	the	corresponding	code	in	PIPE:

They	differ	only	in	the	prefixes	added	to	the	PIPE	signals:	
	for	the
...
from	pipeline	register	M	(signal	M_valA).	When	a	
	instruction	enters
the	write-back	stage,	the	return	address	is	read	from	pipeline	register	W

(signal	W_valM).	All	other	cases	use	the	predicted	value	of	the	PC,
...
out-of-range	instruction	address,	and	we	can	detect	an	illegal	instruction

or	a	halt	instruction.	Detecting	an	invalid	data	address	must	be	deferred
to	the	memory	stage.
Practice	Problem	
4.30	
(solution	page	
...
by	the	instruction	in	the	write-back	stage.
...
results	of	the	instruction	currently	in	the	write-back	stage.
...
consider	is	when	there	is	a	bubble	in	the	write-back	stage.	This	is	part	of

normal	operation,	and	so	we	want	the	status	code	to	be	
	for	this	case
as	well:
Execute	Stage
Figure	
...
Suppose	instead	that	we	use	signal	E_dstE,	the	destination	register	ID	in

pipeline	register	E	for	this	selection.	Write	a	Y86-64	program	that	would
...
cases	for	which	other	mechanisms,	such	as	data	forwarding	and	branch
...
destination	register	is	in	the	decode	stage,	we	want	to	hold	back	the

second	instruction	in	the	decode	stage	and	inject	a	bubble	into	the
...
The	fetch	stage	repeatedly	fetches	the	
	...
	instruction,	but	then	the	pipeline	control	logic	injects	a	bubble	into	the

decode	stage	rather	than	allowing	the	
...
instructions	as	if	nothing	were	amiss.	As	the	excepting	instruction

reaches	the	memory	stage,	we	take	steps	to	prevent	later	instructions
...
bubble	into	pipeline	register	E,	we	want	the	icode	field	to	be	set	to	

and	the	dstE,	dstM,	srcA,	and	srcB	fields	to	be	set	to	the	constant	
.
...
load	instruction	(
	or	
)	in	the	execute	stage,	while	the	other