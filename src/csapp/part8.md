...
them	with	higher-level	helper	functions,	called	
	and
...
there	is	no	such	thing	as	an	EOF	character.	Rather,	EOF	is	a
...
to	request	the	home	page	from	the
AOL	Web	server.
⁁
...
does	not	use	any	of	the	information	in	the	request	headers.	It	simply
...
request	for	dynamic	content	is	served	by	running	a	program	in	the
...
read	buffer	so	that	we	can	call	
	on	the
...
detach	itself	before	it	begins	processing	the	request	so	that	its	memory
...
wrong	answers,	we	get	different	answers	each	time!
-------------------------------------------
code/conc/badcnt.c
...
The	execution	history	of	a	program	is	modeled	as	a	
trajectory
	through	the
state	space.	
Figure	
...
indeed	any	concurrent	program	that	shares	global	data	structures—we
must	somehow	
synchronize
...
without	interruption.	The	increment	operation	in	
V
...
mutex.	Similarly,	performing	the	
V
	operation	is	called	
unlocking
	the
...
trajectory	in	a	progress	graph.	Regardless,	the	message	remains

the	same:	always	synchronize	accesses	to	your	shared	variables,
...
When	we	run	the	properly	synchronized	program,	it	now	produces	the
...
data-dependent	differences	in	the	encoding	and	decoding	times	for

individual	frames.	The	buffer	provides	a	reservoir	of	slots	to	the	producer
...
consumers,	and	
n
	the	buffer	size	in	units	of	items.	For	each	of	the

following	scenarios,	indicate	whether	the	mutex	semaphore	in
	and	
	is	necessary	or	not.
A
.	
...
are	an	unbounded	number	of	concurrent	readers	and	writers.
-------------------------------------------
code/conc/sbuf.c
...
a	customer	who	is	booking	a	seat	must	have	exclusive	access	to	the

database.	As	another	example,	in	a	multithreaded	caching	Web	proxy,	an
...
buffer	in	a	producer-consumer	program.
...
interesting	code	to	study	because	it	shows	you	a	general	technique	for
...
n
	−	1	in	parallel.	Of
course,	there	is	a	closed-form	solution	for	this	particular	problem,	but

nonetheless	it	is	a	concise	and	easy-to-understand	exemplar	that	will
...
Notice	that	we	are	careful	to	protect	each	update	with	
P
	and	
V
	mutex
...
synchronization	is	that	the	main	thread	must	wait	for	all	of	the	children	to
finish.	After	the	peer	threads	have	terminated,	the	main	thread	sums	up
the	elements	of	the	
	vector	to	arrive	at	the	final	result.
-------------------------------------------
...
global	variable	protected	by	a	mutex.
-------------------------------------------
code/conc/psum-mutex.c
...
insight	into	how	well	a	parallel	program	is	exploiting	potential	parallelism.
The	
speedup
...
is	more	difficult	to	measure	than	relative	speedup	because	measuring
...
accomplished	per	unit	time.	For	example,	if	we	can	double	the	number	of
...
thread-unsafe	functions	is	relatively	easy	to	make	thread-safe:	protect

the	shared	variables	with	synchronization	operations	such	as	
P
	and
V
...
safe,	and	thread-unsafe	functions.
...
deep	copy
	of	the
structures	in	order	to	copy	the	entire	structure	hierarchy.	Third,	the	lock-

and-copy	approach	will	not	work	for	a	class	2	thread-unsafe	function
such	as	
...
each	one.	Each	peer	thread	copies	the
-------------------------------------------
code/conc/race.c
...
thread.	Can	you	spot	the	race?	Here	is	what	happens.	When	the	main

thread	creates	a	peer	thread	in	line	13,	it	passes	a	pointer	to	the	local
stack	variable	
i
...
progress	graph	for	a	pair	of	threads	that	use	two	semaphores	for	mutual
...
possible	because	the	overlapping	forbidden	regions	block	progress	in
...
simple	and	effective	rule	to	prevent	deadlocks:
...
Concurrency	introduces	other	difficult	issues	as	well.	Functions	that	are
...
speedup	model	[
43
]	as	an	alternative	to	strong	scaling.
...
one	of	two	different	Pthreads	function	calls.	Which	ones?
-------------------------------------------
...
Use	the	lock-and-copy	technique	to	implement	a	thread-safe	non-
...
However,	if	you	try	this	approach	in	a	concurrent	server	based	on
...
Web	server	based	on	threads.
Your	solution	should	create	a	new	thread	for	each	new	connection
request.	Test	your	solution	using	a	real	Web	browser.
...
remote	server	to	respond	to	a	request	so	that	it	can	serve	one

browser,	it	should	be	working	on	a	pending	request	from	another
browser.
...
equal	to	1.	Thus,	a	single	
	operation	is	sufficient	to	free	the	memory

resources	associated	with	the	connected	descriptor	when	we	are	through
with	it.
Solution	to	Problem	
12.6	
(page
...
Notes:
	...
	A	global	variable	that	is	written	by	the	main	thread	and
read	by	the	peer	threads.
...
pops	the	top	thread	ID	from	the	stack	and	restarts	that	thread.	Given	this
...
In	the	thread	routine,	we	cast	the	argument	back	to	an	
	and
assign	it	to	
:
B
...
The	wrappers	are	packaged	in	a	single	source	file	(
)	that	is
...
Figure
A.1
	shows	the	code	for	the	error-reporting	functions.
...
Report	35,	Digital	Systems	Research	Center,	1989.
...
the	26th	International	Symposium	on	Computer	Architecture	(ISCA)
,
...
,	2008.
[35]	
M.	W.	Eichen	and	J.	A.	Rochlis.	With	micro-	scope	and	tweezers:	An

analysis	of	the	Internet	virus	of	November,	1988.	In	
Proceedings	of
...
the	16th	ACM	Symposium	on	Operating	System	Principles	(SOSP)
,
...
[75]	
S.	A.	Mahlke,	W.	Y.	Chen,	J.	C.	Gyllenhal,	and	W.	W.	Hwu.	Compiler

code	transformations	for	superscalar-based	high-performance
systems.	In	
...
Boneh.	On	the	effectiveness	of	address-space	randomization.	In
...
217
	...
	[x86–64]	move	if	not	unsigned	less,	
217
...
convert	single	precision	to	integer	instruction,	
297
